...
const emit = defineEmits(['filterByName', 'filterByDay', 'filterByCategory']);
...
const isFilteringByCategory = ref(false);
...
function filter() {
    ...
    if (isFilteringByCategory.value) emit('filterByCategory', category.value);
}
</script>
<template>
    <div class="flex flex-column gap-2">
        <div>
            ...
            <button class="inline-flex items-center" @click="filterByCategory">
                <svg viewBox=" 0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6">
                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                    <g id="SVGRepo_iconCarrier">
        ...
        <div v-if="isFilteringByCategory">
            <label for="name" class="p-2">Category</label>
            <select class="select  appearance-none text-black bg-gray-200 w-35" aria-label="select" v-model="category">
                <option>Comida</option>
                <option>Cena</option>
            </select>
        </div>

        <button v-if="isFilteringByName || isFilteringByDay || isFilteringByCategory"
