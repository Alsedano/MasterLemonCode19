...
function applyFilters(meals: Meal[]) {
    ...
    if (filterCategory.value) {
        ...
        meals = meals.filter(m => m.dinner === dinner);
    }
    return meals;
}
...
function showByCategoryLunch(): boolean {
    return filterCategory.value ? filterCategory.value === 'Comida' : true;
}

function showByCategoryDinner(): boolean {
    return filterCategory.value ? filterCategory.value === 'Cena' : true;
}
</script>
...
<template>
    <div class="flex flex-row justify-between items-center w-full">
        ...
        <FilterMeal @filter-by-day="filterByDay" @filter-by-name="filterByName" @filter-by-category="filterByCategory">
        </FilterMeal>
    </div>
        ...
        <div v-for="(day, index) in days" :key="index">
            ...
            <div class="max-w-sm rounded overflow-hidden shadow-lg bg-gray-600" v-if="isFilteredDay(day)">
                ...
                <div class="px-6 py-4">
                    ...
                    <div class="font-bold text-xl mb-2" v-if="showByCategoryLunch()">Comida</div>
                    <div v-if="showByCategoryLunch()">
                        <div v-for="(meal) in getLunchMealByDay(day)" :key="meal.id"
                            ...
                            <button @click="deleteMeal(meal.id)" aria-label="Eliminar comida">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="#615fff" viewBox="0 0 24 24"
                                    class="w-6 h-6">
                                    <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                                    <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
                                    <g id="SVGRepo_iconCarrier">
                    ...
                    <div class="font-bold text-xl mb-2" v-if="showByCategoryDinner()">Cena</div>
                    <div v-if="showByCategoryDinner()">
